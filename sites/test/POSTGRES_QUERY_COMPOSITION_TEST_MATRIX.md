...
- [x] Re-enabled parallel non-composed query execution still passes.
- [x] Queries containing only original positional parameters still preserve numbering.
- [x] Queries mixing original positional parameters and built-in named bindings still preserve numbering.
- [x] Repeated use of the same positional placeholder in one body reuses a single bound value correctly.
- [x] Placeholders inside SQL string literals are never rewritten.
- [x] Placeholders inside SQL comments are never rewritten.
- [x] Placeholders inside double-quoted identifiers are never rewritten.
...
- [x] The same explicit `external` binding referenced multiple times in one SQL body stays stable.
- [x] Explicit `external` where `domain` is a literal string works.
- [x] Explicit `external` where `query` is a literal string works.
...
- [x] Explicit `external` with an empty result set behaves correctly in body consumers.
- [x] Explicit `external` target queries returning more than one column fail with `INVALID FOREIGN QUERY RESULT`.
- [x] Explicit `external` still respects cross-domain authorization restrictions.
- [x] Explicit `external` same-domain recursion routed through another query path fails with `CIRCULAR FOREIGN QUERY`.
- [x] Explicit `external` cross-domain recursion routed through another query path fails with `CIRCULAR FOREIGN QUERY`.
- [x] Explicit `external` does not leak unused original parent params into the final SQL call.
...
- [x] Implicit same-domain references can be consumed via `unnest($query_name::TEXT[])`.
- [x] Repeated implicit use of the same same-domain query in one body stays stable.
- [x] Implicit same-domain references to a missing query fail with `INVALID QUERY '<name>' FOR '<domain>'`.
...
- [x] Hyphenated same-domain query names remain supported in implicit references.
...
- [x] Final rewritten SQL preserves body-level original positional placeholders.
- [x] Final arg compaction removes unused original params from the outermost SQL call.
- [x] Parameter order in the final SQL call follows first use in the rewritten SQL body.
- [x] Queries comparing two rewritten parameters without type context are either rejected in fixtures or explicitly cast.
- [x] Malformed placeholder-like text such as `$1abc` or dollar-quoted strings is not misparsed, if those inputs are allowed.
- [x] Max foreign-query depth is still enforced with `FOREIGN QUERY DEPTH EXCEEDED`.
- [x] Deep but valid explicit or implicit composition chains below the max depth still succeed.
...
1. Explicit `external` where `query` is a literal string works.
2. Explicit `external` where `domain` resolves from another explicit binding works, if intentionally supported.
3. Explicit `external` where `query` resolves from another explicit binding works, if intentionally supported.
4. Explicit `external` cross-domain recursion routed through another query path fails with `CIRCULAR FOREIGN QUERY`.
5. Queries comparing two rewritten parameters without type context are either rejected in fixtures or explicitly cast.
