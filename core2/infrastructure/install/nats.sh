...
# install using result of: curl -sf https://binaries.nats.dev/nats-io/nats-server/v2@v2.10.20 | sh
...
start() {
  ...
  api="https://binaries.nats.dev"

  # package such as "github.com/nats-io/nats-server/v2"
  pkg="github.com/nats-io/nats-server/v2"

  # binary name such as "nats-server"
  bin="nats-server"

  # original_version such as "main"
  original_version="v2.10.20"

  # version such as "main"
  version="7140387"
  ...
  log_info "  URL: $api/binary/$pkg?os=$os&arch=$arch&version=$version"
  ...
  log_info "Installing $bin to $prefix"
    ...
    log_info "  $ curl -sf https://binaries.nats.dev/$pkg@$version | PREFIX=. sh"
