    ...
    if (authority === 'core') JWTVerification('core', token, resolve, reject)
      ...
      try {
        ...
        if (tokenDomain !== domain) reject(`INVALID TOKEN DOMAIN: ${domain} != ${tokenDomain}`)
          ...
          const { userPublicKey, message } = JSON.parse(
          ...
          const { user                   } = await Agent.state(credentialId)
          const { owner: credentialOwner } = await Agent.metadata(credentialId)
          const { ownerPublicKey         } = await Agent.state(user)
          ...
          } = decryptJSON(SECRET_ENCRYPTION_KEY, userPublicKey, message)

          // Prove user info was generated by owner
          const {
            info
          } = decryptJSON(SECRET_ENCRYPTION_KEY, ownerPublicKey, providerEncryptedInfo)

          if (credentialOwner === userOwner && Date.now() - created < 30_000) {
            resolve({ user, provider_id: user, provider: userOwner, info})
          }
          else reject('ERROR VERIFYING CODE')
        }
      }
      catch (error) {
