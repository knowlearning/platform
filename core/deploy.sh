...
if [ -z "$1" ]; then
  ...
  export INSECURE_DEVELOPMENT_CERT=$(cat ./infrastructure/local/.credentials/INSECURE_DEVELOPMENT_CERT)
  export INSECURE_DEVELOPMENT_KEY=$(cat ./infrastructure/local/.credentials/INSECURE_DEVELOPMENT_KEY)
